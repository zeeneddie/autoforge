...
class Feature(Base):
    ...
    # Story tasks: implementation breakdown generated by story-planner agent before coding starts
    # Each task: {"id": int, "name": str, "description": str, "done": bool,
    #             "test_file": str|None, "test_count": int, "test_output": str|None}
    tasks = Column(JSON, nullable=True, default=None)
    # AC quality labels: set by architect agent during sprint-level AC review (pre-coding)
    # List parallel to `steps`: ["auto-testable", "needs-fixture", "human-only", ...]
    ac_labels = Column(JSON, nullable=True, default=None)
...
def _migrate_add_tasks_column(engine) -> None:
    """Add tasks column to features table (story planner breakdown per user story)."""
    with engine.connect() as conn:
        result = conn.execute(text("PRAGMA table_info(features)"))
        columns = [row[1] for row in result.fetchall()]
        ...
        if "tasks" not in columns:
            conn.execute(text(
                "ALTER TABLE features ADD COLUMN tasks TEXT DEFAULT NULL"
            ))
            conn.commit()
...
def _migrate_add_ac_labels_column(engine) -> None:
    ...
    with engine.connect() as conn:
        result = conn.execute(text("PRAGMA table_info(features)"))
        columns = [row[1] for row in result.fetchall()]
        if "ac_labels" not in columns:
            conn.execute(text("ALTER TABLE features ADD COLUMN ac_labels TEXT DEFAULT NULL"))
        if "escalation_reason" not in columns:
            conn.execute(text("ALTER TABLE features ADD COLUMN escalation_reason TEXT DEFAULT NULL"))
        conn.commit()
...
def create_database(project_dir: Path) -> tuple:
    ...
    # Migrate to add tasks column (story planner breakdown)
    _migrate_add_tasks_column(engine)

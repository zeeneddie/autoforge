...
def _get_change_log(project_dir: Path) -> str:
    ...
    try:
        ...
        if last_tag.returncode == 0 and last_tag.stdout.strip():
            ...
            # No tags yet — show all commits (limit to 50)
            ref_range = "HEAD"
...
def _build_retrospective_html(
    ...
    lines.append(f"<p><em>Generated by MQ DevEngine at {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}</em></p>")
    return "\n".join(lines)
...
def _build_feature_comment_html(feature_name: str, passes: bool, change_log: str) -> str:
    ...
    lines = [
        f"<p><strong>MQ DevEngine Sprint Result: {status}</strong></p>",
